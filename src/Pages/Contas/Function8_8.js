...
export default class Function8_8 extends React.Component{
    constructor(props)
    {
        super(props)
    ...
    funcao(){
        ...
        if(isNaN(x)){
            ...
            alert(`O resultado da função F(x) = Sen(${x}) é igual a ${resultado}`)
        }
    }
    ...
    derivada(){
        ...
        if(isNaN(x)){
            ...
            alert(`O resultado da derivada de F(X) = Sen(${x}) é igual a ${resultado}`)
        }
    }
    ...
    integral(){
        ...
        let b = parseFloat(document.getElementById('valorB').value);
            if(isNaN(a) || isNaN(b)){
                ...
                let resultadoFinal = (-Math.cos(b)) - (-Math.cos(a));
                alert(`O resultado da integral de F(X) = Sen(x) é igual a ${resultadoFinal}`);
            }
    }
    ...
    render(){
        return(
            ...
            <form className='form-container'>
                ...
                <input className='input_container' type='number' id="valorX" step='0,01' placeholder="Valor de X" />
                ...
                <div className='Contas'>
                    <h5>Função: F(X) = sen(x) </h5>
                    <h5>Derivada: cos(x) </h5>
                    <h5>Integral: -cos(b) - -cos(a)</h5>
                </div>
            </form>
        </div>
        )
